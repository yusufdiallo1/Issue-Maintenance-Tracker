...
export function IssueDetailSheet({
  ...
  const run = (
    ...
    startTransition(async () => {
      try {
        ...
        if (res && res.ok === false) {
          // A real server rejection (e.g. one-active-issue) — surface it, don't queue.
          show(res.error === "one_at_a_time" ? t("oneAtATime") : t("loadError"), "error");
          return;
        }
  ...
  if (issue.status === "done") {
    ...
    // Awaiting admin approval — no worker action (admin approves on Approvals).
    action = (
      <div className="actionrow one">
    ...
    action = (
      <div className="actionrow one">
        <button className="abtn done" disabled={pending} onClick={() => setCompleteOpen(true)}>
          <Check />
          {t("markFixed")}
        </button>
      </div>
